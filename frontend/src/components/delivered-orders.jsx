...
export default function DeliveredOrders() { // Renamed component from PendingOrders to DeliveredOrders
  ...
  return (
    <div className="p-4 sm:p-6"> {/* Adjusted padding */}
      <h2 className="text-2xl font-bold text-slate-100 mb-6">Delivered / Dispatched Orders</h2> {/* Changed to h2, adjusted text color */}

      {orders.length === 0 ? (
        <p className="text-slate-400 text-center py-8">No delivered or dispatched orders found.</p>
      ) : (
        orders.map((order) => (
          <div key={order._id}
            ...
            <div className="mb-4">
              ...
              <span className={`inline-block px-3 py-1 rounded-full text-sm font-semibold ${
                order.status === "shipped"
                  ? "bg-blue-600/20 text-blue-300 border border-blue-500/30" // Shipped status badge
                  : order.status === "delivered"
                  ? "bg-green-600/20 text-green-300 border border-green-500/30" // Delivered status badge
                  : "bg-slate-600/20 text-slate-300 border border-slate-500/30" // Default/other status
              }`}>
            ...
            {order.status === "shipped" && (
              <button
                ...
                className="bg-green-600 text-white px-5 py-2 rounded-lg hover:bg-green-700 transition-colors duration-200 font-medium shadow-md"
              >
                Mark as Delivered
              </button>
            )}
            {order.status === "delivered" && (
              <span className="text-green-500 font-medium text-sm ml-2">Order successfully delivered!</span>
            )}
          </div>
        ))
      )}
