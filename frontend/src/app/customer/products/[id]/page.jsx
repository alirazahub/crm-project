...
import { ShoppingCart, Loader2, ArrowLeft, Star, Heart, Clock, Truck, Package, ChevronDown } from "lucide-react";
...
export default function ProductDetails() {
  ...
  const productImages = product.images || ["https://via.placeholder.com/400x500/f0f0f0/333?text=Product+Image"];
    ...
    <div className="bg-[#f8f9fa] min-h-screen py-[20px] px-5 ">
      <div className="max-w-[1200px] mx-auto">
        ...
        <nav className="mb-[20px] text-[12px] text-[#999]">
          <Link href="/" className="hover:underline">Home</Link> &gt; <span className="font-semibold">Product details</span>
        </nav>
        ...
        <div className="bg-white rounded-[12px] p-[40px] mb-[30px] grid grid-cols-1 md:grid-cols-2 gap-[40px] shadow-md shadow-gray-400">
              ...
              {productImages.slice(0, 3).map((image, index) => (
                <img
                  ...
                  className={`w-[80px] h-[80px] object-cover rounded-[8px] cursor-pointer border-2 ${
                    index === activeThumbnail ? "border-[#333]" : "border-transparent"
                  }`}
          ...
          <div className="flex flex-col">
            ...
            <h1 className="text-[32px] font-bold text-[#333] mb-[15px]">{product.name}</h1>

            {/* Pricing & Discount */}
            <div className="flex items-center gap-4 mb-[20px]">
              <p className="text-[24px] font-semibold text-[#333]">${product.price}</p>
              {product.originalPrice && product.price < product.originalPrice && (
                <p className="text-lg line-through text-[#999]">${product.originalPrice}</p>
              )}
              ...
              <div className="bg-[#f9f9f9] rounded-[8px]">
                <details open>
                  ...
                  <div className="px-[15px] pb-[15px] text-[#666] text-[14px] leading-relaxed">
                    {product.description || "Loose-fit sweatshirt hoodie in medium weight cotton-blend fabric with a generous, laid-back oversized silhouette. Jersey-lined, drawstring hood, dropped shoulders, long sleeves, and a kangaroo pocket. Wide ribbing at cuffs and hem. Soft, brushed inside."}
                  </div>
                </details>
              </div>
                ...
                <details open>
                  ...
                  <div className="px-[15px] pb-[15px] space-y-3">
                    <div className="flex items-center text-[#666] text-[14px]">
                      ...
                      <span><strong>Dec 50%</strong> - 3-4 Working Days</span>
                    </div>
                    <div className="flex items-center text-[#666] text-[14px]">
                      <Package className="w-5 h-5 mr-3" />
                      <span><strong>Regular Package</strong> - 10 - 12 October 2024</span>
                    </div>
                  </div>
                </details>
              </div>
        ...
        <div className="bg-white rounded-[12px] p-[40px] shadow-md shadow-gray-400 ">
          <h2 className="text-[24px] font-semibold text-[#333] mb-[30px]">Rating & Reviews</h2>
          <div className="flex flex-col md:flex-row items-center md:items-start mb-[30px] space-y-4 md:space-y-0 md:space-x-[50px]">
            <div className="flex flex-col items-center md:items-start">
              <div className="flex items-baseline">
                <span className="text-[72px] font-bold text-[#333]">{product.ratings?.average?.toFixed(1) || "0.0"}</span>
                <span className="text-[24px] text-[#999]"> / 5</span>
              </div>
              <p className="text-[#666] text-[14px] mt-1">({product.ratings?.count || 0} Reviews)</p>
            </div>
          ...
          <div className="flex items-start bg-[#f9f9f9] p-[20px] rounded-[8px] shadow-sm">
            ...
            <div>
              <div className="flex items-center mb-1">
                ...
                <span className="text-[#999] text-[12px] ml-3">13 Oct 2024</span>
              </div>
              <div className="flex mb-2">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} className={`w-4 h-4 ${i < 5 ? "text-[#ffd700] fill-[#ffd700]" : "text-gray-300 fill-gray-300"}`} />
                ))}
              </div>
