...
export default function FeedbackPage() {
  ...
  const { feedbacks, status, error, successMessage, replyLoading } = useSelector(
      ...
      <div className="container mx-auto p-4 relative z-10">
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-12 md:col-span-3 lg:col-span-2">
            <AdminSidebar currentPath="/admin/feedback" />
          </div>
          ...
          <div className="col-span-12 md:col-span-9 lg:col-span-10">
            <div className="flex flex-col min-h-[80vh]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                {[
                  { label: "New Queries", count: feedbacks.filter(f => f.status === "New").length, color: "yellow" },
                  { label: "In Progress", count: feedbacks.filter(f => f.status === "In Progress").length, color: "blue" },
                  { label: "Responded", count: feedbacks.filter(f => f.status === "Responded").length, color: "green" },
                  { label: "Resolved", count: feedbacks.filter(f => f.status === "Resolved").length, color: "gray" },
                ].map((stat, i) => (
                  <div
                    key={i}
                      ...
                      {feedbacks.map((fb) => (
                        <tr key={fb._id} className="hover:bg-slate-800/40">
                          <td className="px-6 py-4">
                            ...
                            <div className="text-sm text-slate-400">{fb.email}</div>
                          </td>
